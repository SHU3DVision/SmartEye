...
* V 1.0.2 UDP  
新增UDP接收功能；  
新增TCP指令发送功能，在UDP下可以通过软件直接调节积分时间、最小信号强度。
...
# Using
0. 相机连接电脑，推荐使用千兆路由器，更稳定，确认**相机IP和电脑IP同一网段**。相机IP一般为**192.168.7.xxx**  

1. 双击SmartEye.exe即可运行，界面如下：  
![](SmartEye/Image/1-1.png)  

2. IP Address输入相机IP地址和端口号，一般端口默认**50660**。PC Config输入电脑IP地址，用于配置UDP目的地址。  

3. 点击**Send**按钮，发送本机IP地址。  

4. 点击**start**按钮，相机开始连续采集图像。为了避免相机长时间工作，默认启动处于休眠状态，需要唤醒。  

5. 点击**Connect**按钮，显示相机传回的图像。  

6. 点击**Stop**按钮，相机停止采集，进入休眠状态。
